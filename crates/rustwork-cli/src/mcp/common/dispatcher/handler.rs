use super::routes::{
    route_data_architecture_tools, route_legacy, route_mcp_protocol, route_p1_handlers,
    route_rustwork_tools,
};
...
pub async fn handle_request(
    ...
    // Try routes in order: MCP protocol → P1 → Rustwork → Data/Arch → Legacy
    let result = if let Some(res) = route_mcp_protocol(method, &request.params, state).await {
        ...
        res
    } else if let Some(res) = route_data_architecture_tools(method, &request.params, state).await {
        res
